...
2. **Emit `blueprints/BP-<id>/BP-<id>_metadata.json`** matching §1 Required Fields, using §1 Worked Example as the structural template. Substitute user inputs for placeholders. Load-bearing details worth restating because their absence breaks import:
   ...
   - Default `deployment_items[].tier_type` is `"plugin"`. Other tier_types (`server`, `terraform`, `tfconfig`, `tfoperation`) require richer sub-schemas — see §1 Enums — and should only be used if the user explicitly asks. **A server tier is `"server"`, not `"provserver"`** — see the warning under §1 Enums. Before emitting any tier_type other than `plugin`, confirm the exact string against what the repo already exports: `grep -h '"tier_type"' blueprints/*/*_metadata.json | sort -u`. Prefer that evidence over this document and over §1 when they disagree, and report the disagreement so the reference gets fixed.
