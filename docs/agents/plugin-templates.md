...
- **Worked example:** `forms/FRM-h4py5w3a` on `resource_actions/RSA-dxrh4m6j` (the shared Terraform Update action). Its variables panel starts empty; the async function `form_functions/FJS-ylyi1tc0` fetches the panel spec from the form-options webhook (`source=day2_panel&resource={object_id}`, built by `tfc_api.day2_form_panel` from the blueprint's order form and the resource's `tfc_var_*` mirrors) and installs it with `panel.fromJSON({templateElements})`. One action and one form serve every blueprint.
